...
class JinjanatorRPC:
    def __init__(self) -> None:
        self._plugin_hook_callers = get_hook_callers()
    ...
    def jinjanate(
        ...
        input_data: StringIO | BufferedReader
        if data.startswith("-"):
    ...
    try:
        while True:
            ...
            try:
                ...
                stdin_wrap = open(os.dup(fd), "rb", closefd=False)  # noqa: SIM115
                stdout_wrap = open(os.dup(fd), "wb", closefd=False)  # noqa: SIM115
                rpc = jsonrpyc.RPC(
                    target=rpc_obj,
                    stdin=stdin_wrap,
                    stdout=stdout_wrap,
                ...
                while True:
                    ...
                    if not line:  # b"" == EOF (client closed the connection)
                        break
                    line_str = line.decode("utf-8").strip()
                    if line_str:
                        rpc._handle(line_str)  # noqa: SLF001
            finally:
                rpc.stdin.close()  # closes the dup'd read fd
                rpc.stdout.close()  # closes the dup'd write fd
                conn.close()
